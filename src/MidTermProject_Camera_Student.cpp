...
int main(int argc, const char *argv[])
{
    ...
    //DETECTOR AND DESCRIPTOR SELECTORS
    string detectorType = "FAST";
    string descriptorType = "ORB";
    //string descriptorType_BF = "DES_BINARY";//WHILE USING BRUTE FORCE MATCHING
    ...
    for (size_t imgIndex = 0; imgIndex <= imgEndIndex - imgStartIndex; imgIndex++)
    {
        ...
        //A simple comparison is done between the predefined buffer size variable and the size of the dataBuffer list. If the latter exceeds the former, the first dataframe of the dataBuffer list is erased. 
        ...
        if (bFocusOnVehicle)
        {
          ...
          {
            if (!vehicleRect.contains(it->pt))//CHECKS IF POINTS ARE CONTAINED IN THE RECTANGLE
            {
              keypoints.erase(it);
            }  
          }    
        ...
        if (dataBuffer.size() > 1) // wait until at least two images have been processed
        {
            ...
            //string descriptorType_BF = "DES_HOG"; // DES_BINARY, DES_HOG
            ...
            //TOTAL NUMBER OF MATCHES ARE CALCULATED AND STORED
      ...
      //TOTAL TIME FOR DETECTION AND DESCRIPTION IS CALCULATED
      	totalTime += detectionTime+descriptorTime;
    } // eof loop over all images

  /*ofstream file;
  file.open ("results.txt", std::ofstream::out | std::ofstream::app);
  file << "Detector---------------| " << detectorType << "\n" << "Descriptor-------------| "<<descriptorType<< "\n"<< "Total Keypoints -------| " << totalKeypoints << "\n" <<  "Total Matches ---------| " << totalMatches<< "\n" << "Total Time ------------| " << totalTime * 1000.0 << "ms \n" << "Ratio -----------------| "<< totalMatches / (totalTime * 1000.0) <<"\n";
  file.close();*/
